{-# LANGUAGE GADTs #-}
module Game.Chess.PGN.Import (readPGNFile, pgn) where
...
lbrace, rbrace, semi, period, quote, backslash, dollar :: Word8
...
nag = lexeme $  single dollar *> L.decimal
            <|> string "!!" $> 3
            <|> string "??" $> 4
            <|> string "!?" $> 5
            <|> string "?!" $> 6
...
comment = (fmap . fmap) (chr . fromEnum) $
      single semi *> manyTill anySingle (eof <|> void eol)
